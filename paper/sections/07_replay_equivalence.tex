...
\paragraph{Proposition 7.4 (Replay equivalence under fixed $I_{\text{exec}}$).}
...
We evaluated replay equivalence on the reference implementation using a fixed
event stream of 1000 DECISION events produced under identical authoritative
inputs and unchanged policy configuration. Using \texttt{dbl\_policy.allow\_all}
as the baseline policy, all 1000 DECISION events reproduced identical digests
under replay.

We then re-evaluated the same recorded event stream under a modified policy
without re-executing any turn. Using the variant policy
\texttt{bench.strict\_extensions}, 200 of 1000 decisions changed from ALLOW to
DENY, while 800 remained unchanged.

These results provide empirical support for replay equivalence at the
governance layer and illustrate that normative outcomes can change under
policy substitution without requiring re-execution of the observational layer.
